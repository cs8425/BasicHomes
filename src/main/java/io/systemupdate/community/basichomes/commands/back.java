...
public class back implements TabExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args){
		...
		if (!sender.hasPermission("basichomes.back")) {
		...
		User userProfile = BasicHomes.instance.userProfiles.get(uuid);
		Location loc = userProfile.getLastDeath();
		if (loc == null) {
			sender.sendMessage(BasicHomes.instance.lang.getText("no-back-point"));
			return true;
		}
		userProfile.clearLastDeath();
		//p0.teleport(loc);
		p0.teleportAsync(loc);
		return true;
	}
}
